#!/usr/bin/env python3
"""Prepare full + crop views for all 436 frozen v4 DEV rows."""
...
CONFIG = ROOT / "protocol/v4_full_dev_execution_plan.json"
MANIFEST = ROOT / "manifests/v4_full_dev_436.csv"
WEIGHT = ROOT / "assets/person_detector/yolo11n.pt"
DETECTIONS = ROOT / "detector/person_detections_full_dev.csv"
CROPS = ROOT / "manifests/v4_full_dev_crop_manifest.csv"
SUMMARY = ROOT / "detector/detector_full_dev_summary.json"
...
def main() -> int:
    ...
    if len(rows) != 436:
        raise RuntimeError("full DEV manifest must contain 436 rows")
    ...
    for index, row in enumerate(rows, 1):
        ...
        full_path = ROOT / "prepared_full_dev" / row["diagnostic_id"] / "full_scene.jpg"
        crop_path = ROOT / "prepared_full_dev" / row["diagnostic_id"] / "person_crop.jpg"
        ...
        detection_rows.append({
            ...
            "ground_truth": row["ground_truth"], "source_width": width,
        ...
        print(f"[{index}/436] {row['diagnostic_id']} detected={chosen is not None}", flush=True)
    ...
    by_ground_truth = {}
    for name in ("positive", "negative", "uncertain"):
        subset = [row for row in crop_rows if row["ground_truth"] == name]
        count = sum(row["person_detected"] == "true" for row in subset)
        by_ground_truth[name] = {"total": len(subset), "detected": count, "coverage": count / len(subset)}
    summary = {
        "status": "PASS",
        ...
        "person_detection_coverage": detected / len(crop_rows),
