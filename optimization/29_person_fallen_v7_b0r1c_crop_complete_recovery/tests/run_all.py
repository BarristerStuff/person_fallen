...
src=(D/'tools/run_v7_b0r1c.py').read_text()
...
assert "V7_B0R1C_" in src
assert "ProxyHandler({})" in src
# no retry loop
assert 'for attempt' not in src and 'retry' not in src.lower()
print('ALL_UNIT_TESTS_PASS')
exec((D/'tests/test_b0r1c_protocol.py').read_text(),{'__file__':str(D/'tests/test_b0r1c_protocol.py'),'__name__':'__main__'})
