...
MANIFESTS = {
    "screen": ROOT / "manifests/person_fallen_v4_operational_screen_crop.csv",
    "val": ROOT / "manifests/person_fallen_v4_operational_val_crop.csv",
}
DETECTOR_SUMMARIES = {
    "screen": ROOT / "detector/screen_detector_summary.json",
    "val": ROOT / "detector/val_detector_summary.json",
}
ATTRIBUTE_KEYS = [
...
def verify_binding(plan: dict, phase: str) -> None:
    ...
    if freeze.get("status") != "FROZEN_PENDING_SCREEN" or freeze.get("candidate") != plan["candidate"]:
        raise RuntimeError("candidate freeze status/binding invalid")
    if freeze.get("plan_sha256") != sha256(PLAN):
        raise RuntimeError("candidate freeze plan SHA mismatch")
    checks = [
        (ROOT / plan["definition"]["path"], plan["definition"]["sha256"]),
        (ROOT / plan["prompt"]["path"], plan["prompt"]["sha256"]),
        (ROOT / plan["policy"]["path"], plan["policy"]["sha256"]),
        (ROOT / plan["temporal_policy"]["path"], plan["temporal_policy"]["sha256"]),
        (ROOT / "assets/person_detector/yolo11n.pt", plan["detector"]["weight_sha256"]),
    ]
    for path, expected in checks:
        if sha256(path) != expected:
            raise RuntimeError(f"frozen candidate SHA mismatch: {path}")
    for key in ("manifest_builder", "detector_runner", "evaluation_runner"):
        tool = plan["execution_tools"][key]
        if sha256(ROOT / tool["path"]) != tool["sha256"]:
            raise RuntimeError(f"frozen execution tool SHA mismatch: {key}")
    source = plan["source_manifests"][phase]
    if sha256(Path(source["path"])) != source["sha256"]:
        raise RuntimeError(f"source manifest SHA changed: {phase}")
    if phase == "val":
        screen_dir = ROOT / "eval/runs/screen_V4-A0-FULL-CROP"
        screen_summary = screen_dir / "summary.json"
        screen_lock = screen_dir / "COMPLETION_LOCK.json"
        if not screen_summary.is_file() or not screen_lock.is_file():
            raise RuntimeError("VAL is not allowed before a completed SCREEN")
        if load_json(screen_summary).get("gate_pass") is not True:
            raise RuntimeError("SCREEN gate failed; VAL is forbidden")
...
def main() -> int:
    ...
    expected_split = "V3_SCREEN" if args.phase == "screen" else "V3_VAL"
    ...
    detector_summary = load_json(DETECTOR_SUMMARIES[args.phase])
    if detector_summary.get("status") != "PASS" or detector_summary.get("sample_count") != len(rows):
        raise RuntimeError("detector summary invalid")
    run_dir = ROOT / "eval/runs" / f"{args.phase}_V4-A0-FULL-CROP"
    ...
    preflight.update({
        ...
        "plan_sha256": sha256(PLAN), "freeze_sha256": sha256(FREEZE),
    ...
    try:
        for index, row in enumerate(rows, 1):
            ...
            request_id = f"{args.phase.upper()}_V4-A0-FULL-CROP_{index:04d}"
        ...
        summary = {"status": "COMPLETE", "revision_id": plan["revision_id"], "candidate": plan["candidate"], "phase": args.phase, "operational_definition_frozen": True, "gt_type": "PROMPT_DERIVED_SYNTHETIC_GT", "model_prediction_used_as_gt": False, "screen_rows_read": len(rows) if args.phase == "screen" else 0, "val_rows_read": len(rows) if args.phase == "val" else 0, "holdout_rows_read": 0, "plan_sha256": sha256(PLAN), "freeze_sha256": sha256(FREEZE), "definition_sha256": sha256(ROOT / plan["definition"]["path"]), "prompt_sha256": sha256(PROMPT), "policy_sha256": sha256(POLICY), "crop_manifest_sha256": sha256(crop_manifest), **evaluated}
